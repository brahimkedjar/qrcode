    ...
    try {
        ...
        if ($idx) {
            ...
            foreach ($kv in $byIndex.GetEnumerator()) {
                ...
                # Exclude pure-primary id-only combos; drop key column if present
                ...
                if ($insertCmdNoKey -ne $null) {
                    ...
                    try { [void]$insertCmdNoKey.ExecuteNonQuery() } catch {
                        ...
                        if ($altId4 -eq $null -or $altId4 -is [System.DBNull]) {
                            # Heuristic: probe by common label/code columns
                        ...
                        # Could not recover; log candidate key values, then skip this row to continue syncing others
                        try {
                        ...
                        Write-Warning ("    Skipping row due to unique-constraint conflict not resolvable by detected natural keys.")
                        continue
                    }
                    $inserted++
                }
